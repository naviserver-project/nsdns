...
dnsPacket *
dnsResolve(char *name,int type,char *server,int timeout,int retries)
{
    fd_set fds;
    int sock,len;
    char buf[DNS_BUFSIZE];
    ...
    if((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
      if(dnsDebug > 3) Ns_Log(Error,"dnsResolve: %s: socket: %s",name,strerror(errno));
      return 0;
    }
    ...
    while(retries--) {
      ...
      FD_SET(sock,&fds);
      if(select(sock+1,&fds,0,0,&tv) <= 0 || !FD_ISSET(sock,&fds)) {
        if(dnsDebug > 3 && errno) Ns_Log(Error,"dnsResolve: %s: select: %s",name,strerror(errno));
        continue;
      }
      if((len = recv(sock,buf,DNS_BUFSIZE,0)) <= 0) {
...
void
dnsRecordDump(Ns_DString *ds,dnsRecord *y)
{
    if(!y) return;
                   ...
                   unsigned long retry,unsigned long expire,unsigned long ttl)
{
    ...
    y->len = 20;
    if(y->data.soa->mname) y->len += strlen(y->data.soa->mname);
    if(y->data.soa->rname) y->len += strlen(y->data.soa->rname);
    y->ttl = dnsTTL;
    return y;
}

Tcl_Obj *
dnsRecordCreateTclObj(Tcl_Interp *interp,dnsRecord *drec)
{
...
dnsRecord *
dnsRecordRemove(dnsRecord **list,dnsRecord *link)
{
...
int
dnsRecordSearch(dnsRecord *list,dnsRecord *rec,int replace)
{
    dnsRecord *drec;

    for(drec = list;drec;drec = drec->next) {
      if(drec->type != rec->type) continue;
      switch(drec->type) {
       case DNS_TYPE_A:
       ...
       case DNS_TYPE_CNAME:
       case DNS_TYPE_PTR:
           if(!rec->data.name || !drec->data.name) return -1;
       ...
       case DNS_TYPE_NAPTR:
           ...
           break;
       case DNS_TYPE_SOA:
           if(!rec->data.soa || !drec->data.soa) return -1;
...
dnsPacket *
dnsPacketCreateReply(dnsPacket *req)
{
    dnsRecord *rec;
    dnsPacket *pkt = NULL;

    if(!req || !req->qdlist) return 0;
    pkt = ns_calloc(1,sizeof(dnsPacket));
    pkt->id = req->id;
    DNS_SET_QR(pkt->u,1);
    DNS_SET_AA(pkt->u,1);
...
dnsPacketLog(dnsPacket *pkt,int level,char *text, ...)
{
    ...
    for(y = pkt->qdlist;y;y = y->next) dnsRecordDump(&ds,y);
    Ns_DStringPrintf(&ds," ANSWER SECTION: ");
    for(y = pkt->anlist;y;y = y->next) dnsRecordDump(&ds,y);
...
dnsPacketAddRecord(dnsPacket *pkt,dnsRecord **list,short *count,dnsRecord *rec)
{
    ...
    if(dnsRecordSearch(*list,rec,0)) return -1;
...
dnsType(char *type)
{
    if(!strcasecmp(type,"ANY")) return DNS_TYPE_ANY;
    if(!strcasecmp(type,"A")) return DNS_TYPE_A;
    if(!strcasecmp(type,"NS")) return DNS_TYPE_NS;
